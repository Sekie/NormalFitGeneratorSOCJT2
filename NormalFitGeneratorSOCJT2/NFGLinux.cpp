...
void RunSOCJT(string inName, string outName)
{
	...
	system(("/storage/htran/mono/bin/mono \"SOCJT2_linux.exe\" " + inName + " " + outName).c_str()); // Executes SOCJT 2.exe with mono on the chemistry Red Hat cluster.
}
...
int NormalFitGenerator(string inFit, string Input, string Output, double StdDev, int N, string strThread)
{
	...
	std::ofstream TotalOutput((Output + ".total.nfgout").c_str()); // This is the output file with each fit value running down a column.
...
void RMSGridScan(vector<string> ParameterName, vector<double> ParameterStart, vector<double> ParameterStep, vector<int> ParameterStepNum, string InputName, string OutputName)
{
	...
	ofstream ScanParameters((OutputName + ".param.scan").c_str());
